...
export async function extendCustomerService(
  ...
  const newExpiredAt = startDate + service.billingCycle * months * 24 * 60 * 60 * 1000;
    ...
    if (targetCustomerId && targetCustomerId !== 'CUST-ANONYMOUS' && paymentType === 'in') {
      if (isDirectPayment) {
        ...
        if (autoGeneratedOrderId && matchedService && autoOrderStatus === 'paid') {
          await extendCustomerService(tx, targetCustomerId, matchedService.id, payment.paidAt, months);
        }
      } else {
        await tx.update(customers)
