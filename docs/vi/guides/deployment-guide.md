...
Các biến bí mật (Secrets) cần được mã hóa an toàn trên hạ tầng của Cloudflare bằng công cụ Wrangler CLI. Nếu bạn chạy deploy và quản lý Worker dưới một tên cụ thể (ví dụ: `reddcom-crm`), bạn nên thêm flag `--name reddcom-crm` vào sau mỗi lệnh.
...
1. **SESSION_SECRET** (Khóa bí mật dùng để ký và giải mã Cookie phiên đăng nhập. Yêu cầu tối thiểu 32 ký tự ngẫu nhiên):
   ...
   *Sau đó nạp vào Cloudflare:*
   ```bash
   npx wrangler secret put SESSION_SECRET --name reddcom-crm
   ```

2. **Tài khoản quản trị viên (Admin) ban đầu:**
   ```bash
   # Tên tài khoản Admin khởi tạo (Mặc định nếu bỏ qua: admin)
   npx wrangler secret put INITIAL_ADMIN_USERNAME --name reddcom-crm
   
   # Mật khẩu tài khoản Admin khởi tạo (Mặc định nếu bỏ qua: admin123)
   npx wrangler secret put INITIAL_ADMIN_PASSWORD --name reddcom-crm
   ```

### Nhóm 2: Tích hợp Cổng thanh toán (SePay Webhook)

Để đối soát tiền tự động qua SePay, bạn **bắt buộc** phải cấu hình khóa bí mật. Khóa này dùng để xác thực chữ ký số webhook gửi đến hệ thống (không có giá trị mặc định/fallback):
```bash
# Token xác thực webhook được thiết lập trên SePay Dashboard (Header Authorization: Apikey <secret>)
npx wrangler secret put SEPAY_WEBHOOK_SECRET --name reddcom-crm
```
...
1. **GITHUB_BACKUP_TOKEN** (Personal Access Token của GitHub, quyền Contents: Read & Write):
   ```bash
   npx wrangler secret put GITHUB_BACKUP_TOKEN --name reddcom-crm
   ```
2. **GITHUB_BACKUP_OWNER** (Tên tài khoản hoặc tên Organization sở hữu repo backup):
   ```bash
   npx wrangler secret put GITHUB_BACKUP_OWNER --name reddcom-crm
   ```
3. **GITHUB_BACKUP_REPO** (Tên repository GitHub được dùng để chứa các bản sao lưu):
   ```bash
   npx wrangler secret put GITHUB_BACKUP_REPO --name reddcom-crm
   ```
4. **GITHUB_BACKUP_BRANCH** (Tên nhánh đích muốn đẩy dữ liệu lên, ví dụ: `main`):
   ```bash
   npx wrangler secret put GITHUB_BACKUP_BRANCH --name reddcom-crm
   ```
...
```bash
npx wrangler deploy --name reddcom-crm
```
Sau khi hoàn tất, Wrangler sẽ cung cấp URL chạy live của bạn (ví dụ: `https://reddcom-crm.your-subdomain.workers.dev`).
